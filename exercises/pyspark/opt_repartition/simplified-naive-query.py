...
def get_partition_info(df):
    ...
    try:
        ...
        avg_size = sum(partition_sizes) / len(partition_sizes) if len(partition_sizes) > 0 else 0
        ...
        print(f"Warning: Could not analyze partition info: {str(e)}")
        return {
            "count": 0,
            "min": 0,
            "max": 0,
...
def print_performance_report(metrics):
    ...
    print(f"  Min/Max/Avg rows per partition: {metrics['initial_partitioning']['min']}/{metrics['initial_partitioning']['max']}/{metrics['initial_partitioning']['avg']:.1f}")
    ...
    print(f"  Min/Max/Avg rows per partition: {metrics['result_partitioning']['min']}/{metrics['result_partitioning']['max']}/{metrics['result_partitioning']['avg']:.1f}")
